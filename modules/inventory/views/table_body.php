...
<?php else: foreach ($products as $p): ?>
    <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors animate-fade-in-up">
        ...
        <td class="p-4 text-center">
            <?php 
                $factor = (float)($p['sale_unit_factor'] ?? 1);
                $displayStock = $factor > 0 ? $p['stock'] / $factor : $p['stock'];
                $stockClass = $displayStock <= $p['min_stock'] ? 'text-red-700 bg-red-100 dark:text-red-300 dark:bg-red-900/30' : 'text-green-700 bg-green-100 dark:text-green-300 dark:bg-green-900/30'; 
            ?>
            <span class="inline-block px-2 py-1 rounded-md text-sm font-semibold <?= $stockClass ?>">
                <?= is_float($displayStock) && floor($displayStock) != $displayStock ? number_format($displayStock, 2) : $displayStock ?> 
                <span class="text-[10px] opacity-75"><?= htmlspecialchars($p['sale_unit_abbr'] ?? 'U') ?></span>
            </span>
        </td>
