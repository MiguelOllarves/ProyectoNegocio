<?php 
$profitBoxClass = $summary['profit'] >= 0 ? 'bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400 border-green-100 dark:border-green-800/50' : 'bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 border-red-100 dark:border-red-800/50';
$profitTextClass = $summary['profit'] >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400';
        ...
        <div class="flex-1 grid grid-cols-2 sm:grid-cols-3 gap-3">
            ...
            <div>
                ...
                <select name="limit" class="w-full px-3 py-2 bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-gray-700 rounded-lg text-sm text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-brand-500 h-10">
                    ...
                    <option value="500" <?= $limit == 500 ? 'selected' : '' ?>>500 filas</option>
                </select>
            </div>
        </div>
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    ...
    <!-- Rendimiento -->
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 p-5 flex items-center hover:-translate-y-1 transition-transform relative overflow-hidden group">
        <div class="w-12 h-12 rounded-xl <?= $profitBoxClass ?> flex items-center justify-center text-xl mr-4 border">
...
<div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
    <div class="p-5 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-gray-50/50 dark:bg-slate-800/50">
        ...
        <a href="<?= BASE_URL ?>reports/print_sales?start=<?= $start ?>&end=<?= $end ?>" target="_blank" class="text-sm text-gray-500 hover:text-brand-600 dark:hover:text-brand-400 transition-colors">
        ...
        <table class="min-w-[700px] w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800 text-sm">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-slate-900/50 transition-colors">
                    ...
                    <?php $gananciaClass = $ganancia >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'; ?>
                    <td class="p-4 text-right font-black <?= $gananciaClass ?>">
    ...
    <div class="p-4 border-t border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-slate-800/50 flex flex-col sm:flex-row items-center justify-between gap-4">
        <p class="text-xs text-gray-500 dark:text-gray-400">
            ...
            (Total de registros: <?= $totalSales ?>)
        </p>
        
        <?php
            $buildUrl = function($p) use ($start, $end, $limit) {
                return BASE_URL . "reports?start={$start}&end={$end}&limit={$limit}&page={$p}";
            };
        ?>
